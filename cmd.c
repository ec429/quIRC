...
#include "config_set.c"
			...
			if (val) {
				free(fname);
				fname = strdup(val);
				add_to_buffer(cbuf, STA, QUIET, 0, false, fname,
					      "/set fname ");
			} else
				add_to_buffer(cbuf, ERR, NORMAL, 0, false,
			...
			if (val) {
				...
				while (*p)
					*p++ = '*';
				add_to_buffer(cbuf, STA, QUIET, 0, false, val,
					      "/set pass ");
			} else
				add_to_buffer(cbuf, ERR, NORMAL, 0, false,
...
CMD_FUN(server)
{
	if (args) {
		...
		if (b >= nbufs) {
			...
			sprintf(dstr, "Connecting to %s on port %s...", server,
				newport);
#if ASYNCH_NL
			...
			if (serverhandle) {
				...
				bufs[cbuf].nick =
				    bufs[0].nick ? strdup(bufs[0].nick) : NULL;
				bufs[cbuf].server = cbuf;
				bufs[cbuf].conninpr = true;
				add_to_buffer(cbuf, STA, QUIET, 0, false, dstr,
					      "/server: ");
				redraw_buffer();
			}
#endif
	...
	if (bufs[cbuf].server) {
		if (!SERVER(cbuf).live) {
			...
			if (nl) {
				...
				add_to_buffer(bufs[cbuf].server, STA, QUIET, 0,
					      false, dstr, "/server: ");
				redraw_buffer();
			} else {
				add_to_buffer(bufs[cbuf].server, ERR, NORMAL, 0,
					      false,
...
#else				/* ASYNCH_NL */
			int serverhandle =
			    irc_connect(SERVER(cbuf).serverloc, newport, master,
					fdmax);
			if (serverhandle) {
			...
			if (SERVER(cbuf).handle) {
				if (LIVE(cbuf)) {
					char privmsg[12 + strlen(dest) +
						     strlen(text)];
					sprintf(privmsg, "PRIVMSG %s :%s", dest,
						text);
		...
		for (b2 = 1; b2 < nbufs; b2++) {
			if ((bufs[b2].server == bufs[cbuf].server)
				...
				if (LIVE(b2)) {
					...
					add_to_buffer(b2, MSG, NORMAL, 0, true,
						      args, SERVER(b2).nick);
					bufs[b2].alert = al;	// and restore it
					bufs[b2].hi_alert = hi;
				} else {
					add_to_buffer(b2, ERR, NORMAL, 0, false,
						      "Tab not live, can't send",
						      "/amsg: ");
				}
			}
	...
	if (!((bufs[cbuf].type == CHANNEL) || (bufs[cbuf].type == PRIVATE))) {
		...
		if (SERVER(cbuf).handle) {
			if (LIVE(cbuf)) {
				...
				while (args[strlen(args) - 1] == '\n')
					args[strlen(args) - 1] = 0;	// stomp out trailing newlines, they break things
				add_to_buffer(cbuf, ACT, NORMAL, 0, true, args,
					      SERVER(cbuf).nick);
			} else {
				add_to_buffer(cbuf, ERR, NORMAL, 0, false,
					      "Tab not live, can't send",
...
{
	if (!args) {
		add_to_buffer(cbuf, ERR, NORMAL, 0, false, "Missing arguments!",
			      "/ignore: ");
	} else {
			...
			if (*arg == '-') {
				...
				if (del) {
					...
					char expr[16 + strlen(isrc) +
						  strlen(iusr) + strlen(ihst)];
					sprintf(expr, "^%s[_~]*!%s@%s$", isrc,
						iusr, ihst);
					name *new =
					    n_add(&bufs[cbuf].ilist, expr,
						  bufs[cbuf].casemapping);
					if (new) {
						add_to_buffer(cbuf, STA, QUIET,
							      0, false,
...
CMD_FUN(mode)
{
				...
				if (LIVE(cbuf)) {
					if (bufs[cbuf].type == CHANNEL) {
						char mmsg[8 +
							  ...
							  strlen(modes) +
							  strlen(user)];
						sprintf(mmsg, "MODE %s %s %s",
							bufs[cbuf].bname, modes,
							user);
 ...
 youmode:
			if (bufs[cbuf].type == CHANNEL) {
				if (bufs[cbuf].us) {
					char mm[20 + strlen(SERVER(cbuf).nick) +
						bufs[cbuf].us->npfx];
					int mpos = 0;
	...
	if (args) {
		if (!SERVER(cbuf).handle) {
			...
			if (force || LIVE(cbuf)) {
				...
				add_to_buffer(cbuf, STA, NORMAL, 0, false, args,
					      "/cmd: ");
			} else {
				add_to_buffer(cbuf, ERR, NORMAL, 0, false,
					      "Tab not live, can't send",
